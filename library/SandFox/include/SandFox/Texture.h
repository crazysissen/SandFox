...
namespace SandFox
{
	...
	public:
		...
		virtual void Load(unsigned char* data, int width, int height, bool immutable = true, int stride = 4, DXGI_FORMAT format = DXGI_FORMAT_R8G8B8A8_UNORM, D3D11_BIND_FLAG bindFlags = D3D11_BIND_SHADER_RESOURCE, int srvFormatOverride = -1);
	...
	public:
		...
		virtual void Load(unsigned char* data, int width, int height, bool immutable = false, int stride = 4, DXGI_FORMAT format = DXGI_FORMAT_R8G8B8A8_UNORM, D3D11_BIND_FLAG additionalBindFlags = D3D11_BIND_SHADER_RESOURCE, int srvFormatOverride = -1) override;
